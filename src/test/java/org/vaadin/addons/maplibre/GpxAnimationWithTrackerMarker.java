...
@Route
public class GpxAnimationWithTrackerMarker extends VerticalLayout {
    private ScheduledFuture<?> scheduledFuture = null;

    public GpxAnimationWithTrackerMarker() {
        ...
        try {
            ...
            TrackerMarker trackerMarker = new TrackerMarker(map);
            trackerMarker.setCharacter("M".charAt(0));
            trackerMarker.setColor(NamedColor.DARKBLUE);
            trackerMarker.addPoint(start.getLongitude().doubleValue(), start.getLatitude().doubleValue());
            map.flyTo(trackerMarker.getMarker().getGeometry(), 14.0);
            ...
            scheduledFuture = executor.scheduleWithFixedDelay(() -> {
                ...
                ui.access(() -> {
                    trackerMarker.addPoint(new Coordinate(current.getLongitude().doubleValue(), current.getLatitude().doubleValue()));
                });
